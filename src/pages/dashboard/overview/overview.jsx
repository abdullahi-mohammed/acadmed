...
import { PiAtom, PiBackpack, PiBook, PiCalendar, PiHand, PiStethoscope } from "react-icons/pi"
...
export default function Overview() {
  ...
  return (
    <div>
      ...
      <div className="py-2 grid lg:grid-cols-4 md:gris-cols-3 grid-cols-2 gap-2">
        <TaskType title={"General"} icon={<PiAtom />} schedules={schedules}/>
        <TaskType title={"Study"} icon={<PiBook />} schedules={schedules}/>
        <TaskType title={"Checkup"} icon={<PiStethoscope />} schedules={schedules}/>
        <TaskType title={"Fitness"} icon={<PiHand />} schedules={schedules}/>
      </div>
              ...
              <h3 className="font-semibold border border-transparent border-b-gray-500/[0.2] pb-4">Checkup Results</h3>
                  ...
                  
                  {
                      checkup?.conditions?.map(condition => (
                              <div 
                                key={condition.id}
                                className={`flex flex-col p-4 gap-2 rounded-[10px] my-2 border w-full
